...
class TestRunVC: UIViewController {
    ...
    @IBAction func stopBtnPressed(_ sender: Any) {
        ...
        self.user?.pushTestRun()
        self.user?.emptySignalDictionaries()
    }
    
    @IBAction func nextBtnPressed(_ sender: Any) {
        self.performSegue(withIdentifier: "showStartStage1VC", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? StartStage1VC{
