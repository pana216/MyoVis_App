...
class Stage9ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(updateCounter(notification:)), name: counterNotificationName, object: nil)
        super.viewDidLoad()
    ...
    @IBAction func nextBtnPressed(_ sender: Any) {
        ...
        self.performSegue(withIdentifier: "showKombinatorikFinal", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? KombinatorikFinalViewController{
